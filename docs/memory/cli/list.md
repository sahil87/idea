...
- A stale **done** `[x]` keeps its green — the explicit state signal outranks the age hint, and the span structure (dim spans around the checkbox) stays intact. The combination is unreachable via `list` (`--stale` implies open-only and the default filter is open-only), but the render seam handles it.
