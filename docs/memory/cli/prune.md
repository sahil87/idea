...
The three local flags are `--yes`/`-y`, `--force`, and `--full`; the command takes no positional args (`cobra.NoArgs`), inherits the persistent `--file`/`--main`/`--system` from root (Constitution III), and defines no alias. `--yes`/`-y` and `--force` are **equivalent, additive consent aliases** — `260717-9uh7-toolkit-standards-conformance` added `--yes`/`-y` to conform to toolkit principle №1 (which names `--yes`/`-y` as the canonical flag-satisfiable consent flag) while retaining `--force` unchanged (Constitution VI: the public CLI surface is a growing contract, no renames/removals). The `RunE` computes `consent := force || yes` and passes it into the existing `idea.Prune(path, consent)` force path — no `internal/idea` signature change; the internal `Prune` still takes a single `force bool` that now means "consent given by either flag". Both the immediate-delete branch and the count-only output key on `consent`, not on `force` alone. The `-y` shorthand is collision-free (`-h`/`-v`/`-f`/`-m`/`-s`/`-a` were the pre-existing shorthands).

## TTY-aware decision matrix (consent × TTY)

Behavior is gated on **stdout being a TTY** and consent — the bare invocation is a **free dry run** on a pipe but becomes an **interactive confirm** on a terminal (since `260613-kfcl`), so the un-buryable `[y/N]` prompt is the last line at the cursor instead of a hint that scrolls off-screen.
...
`prune` deliberately gained **no** explicit `--dry-run` flag in the toolkit-standards audit (`260717-9uh7`), even though principle №5 requires destructive writes to support an accurate preview. Its bare (non-consent) invocation is already a de-facto dry run — a free piped dry run + an interactive pre-confirm listing, both routed through the live `Prune` collection path — so the preview obligation is already satisfied; a redundant `--dry-run` verb would duplicate existing semantics without new capability. Recorded as an audit judgment (see the conformance report), not a gap. Contrast `rm`, which had no preview at all and so gained `rm --dry-run` via the `RmPreview` seam (see `structure.md` § Consent & dry-run on destructive writes).
