...
| confirmations — `Added:` (via `idea.EscapeText` in `cmd/idea/add.go`); `Updated:`/`Done:`/`Removed:`/`Reopened:` (via `FormatLine`) | escaped single line per acted idea — batch `done`/`rm` print one line per acted idea in argument order — stdout stays machine-parseable (Constitution VI) |
...
**Backfill stderr notice (Constitution IV split).** The backfill count flows up to the command layer: the mutating internal ops return `(Idea, int, error)` (`Reopen`, `Edit`) or, for the batch verbs, `([]Idea, int, error)` (`Done`, `Rm` — see § Query resolution). When count > 0, the `cmd/idea` layer prints `note: stamped today's date on N previously-dateless item(s)` to **stderr** via the `printBackfillNotice` helper (`main.go`, using `cmd.ErrOrStderr()`); it is suppressed entirely at count 0. stdout stays the machine-parseable confirmation only (Constitution Principle VI). `internal/idea` writes nothing to stderr — output-channel policy lives in `cmd/` per Principle IV. Advisory-notes-to-stderr is the established channel policy: the backfill notice, the batch `done`/`rm` dedupe note (`note: N duplicate query(ies) matched an already-selected idea; acted once` — see § Query resolution), `prune`'s dry-run confirm hint (`Re-run with --yes (or --force) to confirm.` — see `prune.md`), `idea edit`'s editor-form no-op note (`note: text unchanged — nothing to do` — see `edit.md`), and `idea fmt`'s entire report (adoption lines + summary counts — see below) all follow it.

The behavior contract is documented for external consumers in `../../specs/backlog-format.md` and `../../specs/overview.md`.

### Query resolution (`Match` / `FindAll` / `RequireSingle` / `resolveMany`)

`internal/idea/idea.go` owns the query-matching layer shared by every command that takes a `<query>` (`show`/`done`/`reopen`/`edit`/`rm` — `done` and `rm` accept one-or-more `<query>...` and route through the batch resolver below). Three contracts live here, deliberately split:
...
- **`resolveMany` — shared all-or-nothing batch resolution** (260816-xn2j). The unexported `resolveMany(f *File, queries []string, filter FilterKind) ([]int, error)` resolves every query **independently** by calling `RequireSingle` verbatim — same case-insensitive substring matching, same exact-ID-beats-substring precedence, same per-command filter (`FilterOpen` for `done`, `FilterAll` for `rm`), including the guarantee that a filtered-out exact-ID idea is never force-selected. The first no-match or ambiguous query aborts the whole invocation with that query's **existing** error (including the `Multiple matches:` listing), exit 1, backlog byte-untouched — resolve-everything-then-mutate means no partial mutation ever. Two queries resolving to the same idea (e.g. its ID plus a substring of its text) act on it once: dedupe is keyed on the resolved idea index and preserves first-occurrence (argument) order. The multi-resolve loop lives in `internal/idea`, not `cmd/` (Constitution IV); the exported consumers `Done(path string, queries []string) ([]Idea, int, error)`, `Rm(path string, queries []string, force bool) ([]Idea, int, error)`, and `RmPreview(path string, queries []string) ([]Idea, error)` take `queries []string` in place (no parallel `*Many` variants exist — see Design Decisions), so a single-query invocation behaves exactly as it always has (same stdout confirmations, errors, exit codes — batch is purely additive, Constitution VI).

  Output contract for the batch verbs: each acted idea prints its own confirmation line on **stdout** — `Done: {FormatLine}` / `Removed: {FormatLine}` — one per acted idea in argument order (first occurrence for deduped queries), keeping stdout line-per-record scriptable. A successful batch performs exactly **one** `SaveFile` (one atomic write, one whole-file normalization), so the backfill advisory prints **at most once** per invocation. `Rm` captures the removed ideas in first-occurrence order for output, then removes via the existing `removeIdeaAt` seam in **descending idea-index order** so earlier removals cannot invalidate later indices (see Design Decisions). When queries collapse onto the same idea, the cmd layer derives the dedupe count as `len(queries) − len(acted)` (valid because all-or-nothing guarantees every query resolved to exactly one idea) and prints `note: N duplicate query(ies) matched an already-selected idea; acted once` on **stderr** via `printDedupeNotice` (`cmd/idea/done.go`, shared with `rm.go`) — the advisory-notes-to-stderr channel policy; stdout stays per-item lines only. Both commands declare `Use: "done <query>..."` / `"rm <query>..."` with `Args: usageArgs(cobra.MinimumNArgs(1))`, so zero positionals is a usage error (exit 2 — see § Exit-code convention). `idea reopen` stays single-query (`ExactArgs(1)`) — batch reopen is a deliberate non-goal.

### Consent & dry-run on destructive writes (`rm` / `prune`)

The destructive `rm` (batch-capable — one-or-more `<query>...`) and bulk `prune` commands both gate deletion behind an explicit consent flag, and the flag surface is `--yes`/`-y` **and** `--force` — equivalent, additive aliases (toolkit principle №1 names `--yes`/`-y` as the canonical flag-satisfiable consent flag; 260717-9uh7). `--force` is retained alongside it — the public CLI surface is a growing contract (Constitution VI: no renames/removals) — so consent handling in each `RunE` is simply `consent := force || yes` passed into the `idea.Rm`/`idea.Prune` force path. **One consent flag covers the whole batch** — a single `--yes` consents to every resolved removal in the invocation. The `-y` shorthand is collision-free against the other shorthands (`-h`/`-v`/`-f`/`-m`/`-s`/`-a`).

- **Refusal wording leads with the standard flag.** `rm` without consent refuses via `idea.Rm`'s error `Use --yes (or --force) to confirm deletion` (`internal/idea/idea.go`) — the consent check runs **before** any query resolution, so a no-consent batch removes nothing and never touches the resolver; `prune`'s piped non-consent hint is `Re-run with --yes (or --force) to confirm.` (`cmd/idea/prune.go`, stderr). Both name `--yes` first, `--force` as the parenthetical equivalent — the standard's flag leads.

- **`rm --dry-run` — accurate preview on the live match path.** `rm` supports `--dry-run` (toolkit principle №5: a destructive write MUST support an accurate preview that shares the real code path; 260717-9uh7). It routes through the `idea.RmPreview(path string, queries []string) ([]Idea, error)` seam in `internal/idea/idea.go`, which resolves **every** query via the **same** `LoadFile` + `resolveMany(f, queries, FilterAll)` path the live `Rm` uses, and returns all would-be-removed ideas **without writing the file** (260816-xn2j). So an ambiguous or unmatched query is refused identically to the live delete — the preview can never drift from live behavior (the exact drift the standard warns against; a separate preview matcher was rejected for this reason). The `rm` `RunE` prints one `idea.FormatLine(match)` per match to stdout (argument order, first occurrence for deduped queries, with the same `printDedupeNotice` stderr advisory) and writes nothing. `--dry-run` needs no consent (a preview is non-destructive) and **wins over `--force`/`--yes`** — combining them still performs no deletion (the dry-run branch returns before the consent branch is reached). Regression coverage: `TestRmPreview` (`internal/idea/idea_test.go`, table-driven real temp dirs — match returns idea + file byte-identical, ambiguous/no-match refuse) and `TestRmPruneConsent_CLI` (`cmd/idea/main_test.go`, subprocess — `--yes`/`-y` delete like `--force`, `rm --dry-run` prints + leaves the backlog byte-identical, dry-run-wins-over-`--yes`).
...
The README's toolkit blockquote is byte-identical to the readme-extraction standard's canonical line `> Part of [HexoKit](https://hexokit.com) — see all projects there.` — the standard as revised by `sahil87/shll#98`: HexoKit names the toolkit's dashboard product and the family is the HexoKit toolkit, while `shll` stays the toolkit-manager command and `shll standards` the standards reader (260911-jmkb). Toolkit-name prose elsewhere (the README install section, `docs/site/install.md`, and the constitution's § Toolkit Standards article opening) and every `shll.ai` URL name the shll toolkit and shll.ai: shll.ai is the live consuming site until the hexokit.com cutover, and those surfaces flip with the site cutover rather than per repo (`sahil87/shll#56` fixed the shll-toolkit wording; 260718-92gj). This repo carries no run-kit product mention on any live surface, so the HexoKit banner sweep touched only the blockquote. Identifiers name the GitHub owner, not the toolkit — the `sahil87/tap` formula, `github.com/sahil87/…` URLs, and the `sahil87/shll` canonical-source reference stay verbatim.
...
### Batch verbs change signatures in place (no parallel `*Many` variants)
**Decision**: `Done`, `Rm`, and `RmPreview` accept `queries []string` and return `[]Idea` (e.g. `Done(path string, queries []string) ([]Idea, int, error)`); the shared `resolveMany` resolver stays unexported.
**Why**: `internal/idea` has a single `cmd/` consumer — no external Go API contract — and one code path keeps single-query and batch behavior provably identical (single-query invocations stay byte-identical to the pre-batch contract, Constitution VI).
**Rejected**: parallel `DoneMany`/`RmMany` functions — duplicated utilities (code-quality anti-pattern) whose two paths could drift.
*Introduced by*: `260816-xn2j-batch-done-rm-queries`

### Dedupe advisory computed at the cmd layer
**Decision**: `resolveMany` returns deduped indices only; the cmd layer derives the dedupe count as `len(queries) − len(acted)` and prints the stderr advisory itself (`printDedupeNotice` in `done.go`, shared with `rm.go`).
**Why**: keeps `internal/idea` stderr-free (Constitution IV output-channel split), and all-or-nothing resolution guarantees every query resolved to exactly one idea, so the caller can derive the count without growing the return signature with dedupe metadata.
...
**Decision**: `Rm` captures the removed ideas in first-occurrence order for output, then removes them via the existing `removeIdeaAt` seam in **descending idea-index order**.
