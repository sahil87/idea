...
The destructive single-item `rm` and bulk `prune` commands both gate deletion behind an explicit consent flag, and the flag surface is `--yes`/`-y` **and** `--force` — equivalent, additive aliases (established by `260717-9uh7-toolkit-standards-conformance` to conform to toolkit principle №1, which names `--yes`/`-y` as the canonical flag-satisfiable consent flag). `--force` is retained unchanged — the public CLI surface is a growing contract (Constitution VI: no renames/removals), so consent handling in each `RunE` is simply `consent := force || yes` passed into the existing `idea.Rm`/`idea.Prune` force path (no `internal/idea` signature change for the alias). The `-y` shorthand is collision-free against the pre-existing shorthands (`-h`/`-v`/`-f`/`-m`/`-s`/`-a`).
...
`260717-9uh7-toolkit-standards-conformance` was the first full audit against these standards. It fixed the mechanical `help-dump` violation (the `captured_at` removal above), added the `--yes`/`-y` consent alias and `rm --dry-run` for principles №1/№5 (above), and corrected the README/`docs/site` command-reference URL (see `../release/pipeline.md`). Two gaps were deferred to `fab/backlog.md` rather than fixed: **`[3q43]`** — adopting the `skill` standard (deferred per the standard's phased per-repo adoption, principle №10 is a SHOULD so absence was not yet a violation; **since adopted** by `260717-3q43-adopt-toolkit-skill-standard`, which added the visible `idea skill` command — see [skill](skill.md)); and **`[xvsj]`** — a tree-wide usage-error exit-code-`2` convention (**since closed** — see below). The conformance report artifact lives at the change folder's `conformance-report.md`.

**`[xvsj]` closed** by `260717-xvsj-usage-error-exit-codes`, which adopted principle №4's tree-wide 0/1/2 exit-code convention (see § Exit-code convention below). It tagged **both** usage-error seams in one change — the flag-error seam (root `SetFlagErrorFunc`) and the arg-validation seam (`usageArgs`-wrapped validators) — routed through one `main()` mapping, deliberately avoiding the flag-only partial that would have made usage-error classes disagree (flags→2, arg-count→1). With `[3q43]` adopted as well (above), both gaps deferred by the 9uh7 audit are now closed.

The toolkit's standardized name is **shll toolkit** (`sahil87/shll#56`). `260718-92gj-shll-toolkit-rename-conformance` conformed this repo to the renamed name: the README's toolkit blockquote is byte-identical to the readme-extraction standard's post-rename canonical line `> Part of the [shll toolkit](https://shll.ai) — see all projects there.`, and the remaining toolkit-name prose (README, `docs/site/install.md`, and the constitution's § Toolkit Standards article opening) uses "shll toolkit". Identifiers were untouched — the `sahil87/tap` formula, `github.com/sahil87/…` URLs, and the `sahil87/shll` canonical-source reference all name the GitHub owner, not the toolkit, and stay verbatim.

### Exit-code convention (0 success / 1 operational / 2 usage)

Toolkit principle №4 (*Fail fast with actionable errors*, a MUST) fixes the exit-code convention at **0** success / **1** operational failure / **2** usage error. `260717-xvsj-usage-error-exit-codes` adopted it tree-wide, closing the `[xvsj]` deferral above. Before it, `main()` collapsed both failure classes to a uniform `os.Exit(1)`, so an unknown flag, a wrong arg count, and a genuine operational failure were indistinguishable to a caller — and only `shell-init` returned 2 (via its own inline `os.Exit(2)`, a one-off, not a convention).
...
**Two seams tag every usage error** — because cobra's `SetFlagErrorFunc` catches flag-parse errors but **not** arg-count errors (verified at 9uh7), a complete fix must tag both:
...
**No error-wording changes anywhere** — this change altered exit codes only. The `ERROR:` prefix, all refusal/hint texts, and every stderr composition stay byte-identical (9uh7 already assessed the what/why/next wording conformant).

**External contract note.** Usage errors moved `1 → 2`. Callers branching `0` vs non-zero are unaffected; callers pattern-matching exit 1 for usage cases now see 2 — the intended point of the convention (the same migration every toolkit tool makes). Constitution VI freezes stdout schemas and IDs; exit codes are governed by the toolkit standard the constitution binds (§ Toolkit Standards).
...
`main()` is then a thin wrapper: `newRootCmd().Execute()` with the `errSilent` sentinel handling (`errors.Is(err, errSilent)` skips the `ERROR:` line) followed by the two-class exit mapping — `errors.As(err, &uerr)` for `*usageError` → exit **2**, every other error → exit **1**, success → 0 (see § Exit-code convention below). This replaced the former uniform `os.Exit(1)`-on-any-error.
...
**Envelope** (`helpDump`, in field/JSON order): `tool` (literal `"idea"`), `version` (read from `cmd.Root().Version`, the ldflags-stamped value — never hardcoded), `schema_version` (the `helpSchemaVersion` const, `1`), `root` (`buildNode(cmd.Root())`). The envelope is **exactly `{tool, version, schema_version, root}`** and deliberately does **not** carry a `captured_at` field: the toolkit help-dump standard forbids it ("rule with teeth") because the capture timestamp is owned by shll.ai's puller — a tool cannot know its own capture time; the puller stamps it after capture. `260717-9uh7-toolkit-standards-conformance` removed the former `CapturedAt string` field and its `time.Now().UTC()` population (dropping the now-unused `time` import), and inverted the pinning test to assert `captured_at` is **absent** from the raw JSON rather than parsing it as RFC3339 (`help_dump_test.go` no longer references the removed field). `schema_version` stayed `1` — the envelope shape is a frozen cross-repo contract and a field removal is a contract change made only because the standard mandates it.
