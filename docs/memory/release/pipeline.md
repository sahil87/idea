...
Beyond the `help-dump` JSON, shll.ai also **pulls and renders this repo's `README.md` and `docs/site/**` tree daily** to build the `idea` tool page under the `https://shll.ai/idea/*` URL scheme (README → `/idea/readme`; the command reference → `/idea/commands/`; `docs/site/<page>.md` → `/idea/<page>`, e.g. `install.md` → `/idea/install`, `workflows.md` → `/idea/workflows`, and the agent usage bundle `skill.md` → `/idea/skill`, added by `260717-3q43-adopt-toolkit-skill-standard` — see `../cli/skill.md`). The older `/tools/idea/*` scheme is a **redirect stub** (returns a tiny `Redirecting to: /idea/…` page, not the rendered content) and MUST NOT be used in links — `260717-9uh7-toolkit-standards-conformance` fixed the README's command-reference link from `https://shll.ai/tools/idea/commands/` to the canonical `https://shll.ai/idea/commands/` after verifying live which form renders (readme-extraction standard). The pull is mechanical and verbatim — nothing is pushed or hand-fixed — so **this repo's file structure is the only thing that controls whether the page renders cleanly**. The README's leading blockquote text is brand-owned by the readme-extraction standard (the HexoKit line — see [structure](/cli/structure.md) § Toolkit-standards conformance); the extractor matches any leading blockquote, so a blockquote text change needs no pipeline-side change (260911-jmkb). Editing-time gotchas a contributor will trip over:
...
- **260608-3ra7 — conformed the repo to shll.ai's README-extraction contract.** Absolutized the README's external `docs/specs/` links (relative links 404 on the rendered page) and added a `docs/site/**` tree (`install.md`, `workflows.md`) that shll.ai pulls and renders at `/idea/install` and `/idea/workflows` (the current `/idea/*` scheme; the `/tools/idea/*` form these were originally documented under is now a redirect stub — see below). *Repo structure only* — no Go code, tests, CLI behavior, or CI changed. The durable editing constraints this imposes (absolute external links, no reserved page slugs, no mermaid/theme-fragments) are captured above under "shll.ai tool page: README + `docs/site/**` are also pulled and rendered"; the authoritative rules stay in the shll.ai contract, not duplicated here.
- **260717-9uh7 — toolkit-standards conformance touch on the URL scheme.** As part of the full toolkit-standards audit, corrected the README's command-reference link to the canonical `https://shll.ai/idea/commands/` (the `/tools/idea/commands/` form is a live redirect stub). Verified live with `curl`: the standard's form is the ~134 KB rendered page (`<title>Commands | shll</title>`); the old form is a 345-byte redirect stub. The broader audit (help-dump `captured_at` removal, `--yes`/`-y` + `rm --dry-run`) is documented in `../cli/structure.md` § Toolkit-standards conformance.
- **260720-zsar — install-composition standard (Policy B) conformance on the install page.** The rendered `/idea/install` page (`docs/site/install.md`) now points at the shll.ai bootstrap as the recommended path — `curl -fsSL https://shll.ai/install | sh -s -- idea` (subset form) and the full-toolkit form, with `shll install idea` as the equivalent — and its former per-formula "Homebrew tap (recommended)" section carrying `brew install sahil87/tap/idea` is gone. Policy B of the toolkit's install-composition standard bans documenting `brew install sahil87/tap/<tool>` per-repo (individual formula installs stay *supported*; only *documenting* them per-repo is not) and centralizes the install story on https://shll.ai, so per-repo copies no longer drift as the bootstrap evolves. Homebrew remains the managed lifecycle underneath (upgrades/pinning/uninstall via `brew`, `idea update` self-upgrade) — the page keeps that note; only the per-formula install *instruction* was removed. README already conformed (its curl bootstrap *is* the pointer, fixed under `260717-9uh7`); `docs/site/skill.md`'s incidental "self-update via Homebrew" mention was kept (Policy B bans install instructions, not mechanism mentions). *Docs-only* — no Go code, tests, CLI behavior, CI, or release-pipeline contract changed.
